...
describe('getRelease', () => {
  ...
  it('unknown release', async () => {
    await expect(installer.getRelease('foo')).rejects.toThrow(
      new Error(
        'Cannot find Mage release foo in https://raw.githubusercontent.com/step-security/mage-action/release/.github/mage-releases.json' // update ref to main branch post release
      )
    );
  });
});
